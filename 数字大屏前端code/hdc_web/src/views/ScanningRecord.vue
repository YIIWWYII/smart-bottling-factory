	...
	<div class="user-root">
		<div style="text-align: left; width: 100%;">
			...
			<el-table :data="tableData" border stripe style="width: 98%; margin-left:1%; margin-bottom:15%;"
				...
				<el-table-column prop="typeName" label="扫码类型" minWidth="150" fixed></el-table-column>
				<el-table-column prop="createAt" label="扫码日期" minWidth="150" fixed></el-table-column>
		...
		methods: {
			...
			searchBtn(needResetPage, needRefeshCondition) {
				...
				if (this.dateValue) {
					...
					this.startDateParam = startYear + '-' + this.add0(startMonth) + '-' + this.add0(
						startDay) /*+ ' 00:00:00'*/ ;
				} else {
					this.startDateParam = '';
				}
			...
			getDeviceData() {
				Service({
					url: '/api/web/pageBars', //todo
					data: {
						barCode: !this.inputIDForSearch ? null : this.inputIDForSearch,
						currentPage: this.currentPage,
						// page: this.currentPage,
						pageSize: this.pageSize,
						groupId: Local.getId(),
						date: !this.startDateParamForSearch ? null : this.startDateParam,
						// typeCode: this.selectedEquipmentTypeForSearch == '' ? null : this.selectEquipmentType[this.selectedEquipmentTypeForSearch].value,
						productCode: this.selectedEquipmentTypeForSearch
					}
				}).then(res => {
					...
					for (let i = 0; i < res.records.length; i++) {
						this.tableData.push({
							...
							typeName: res.records[i].action == 0 ? '出库' : '入库',
							modelName: res.records[i].model.code,
							createAt: res.records[i].date,
							productType: res.records[i].type.name,
						});
					}
