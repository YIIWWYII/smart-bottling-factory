...
- 客户端身份：`DISPLAY_CLIENT`、`TERMINAL_CLIENT`、`ADMIN_CLIENT`、`AI_CLIENT`，由安装配置或安全存储提供，决定应用可访问的接口范围。
- 操作员身份：`VIEWER`、`OPERATOR`、`ENGINEER`、`ADMIN`，通过 `/auth/login` 获得短期令牌，决定业务写权限。

展板使用 `DISPLAY_CLIENT` 只读，不弹用户登录。小屏使用 `TERMINAL_CLIENT` 可只读；首次写操作必须要求操作员登录，令牌过期后恢复只读。管理端和 AI 协同端始终要求用户登录。AI 协同端的普通问答和资料提交不具备设备控制权限。开发演示配置可简化客户端凭据，但生产配置不得匿名开放控制接口。

## 参数分层

| 类型 | 示例 | 小屏 | 管理端 | AI 协同端 | 后端 |
| --- | --- | --- | --- | --- | --- |
| 实时遥测 | 当前温度、VOC、速度 | 只读 | 查询/历史 | 决策上下文只读 | 接收并持久化 |
| 运行设定值 | 清洗时间、目标温度、灌装量、速度上限 | 在批准范围内人工修改 | 查看、审批范围 | 只显示 AI 字段级建议 | 最终校验并下发 |
| 配方模板 | 某瓶型的整套设备参数 | 选择/应用获批模板 | 创建、版本、审批、发布、回滚 | 只读引用 | 持久化并校验适用性 |
| 人工覆盖 | 人工锁定灌装流量 | 创建/释放授权锁 | 查询、强制释放和审计 | 显示被跳过字段及原因 | 字段级强制锁和版本控制 |
...
- 拥有 `KNOWLEDGE_SUBMIT` 权限的 OPERATOR/ENGINEER/ADMIN：从 AI 协同端提交资料。
- 拥有 `KNOWLEDGE_REVIEW` 权限的审核人：只在后台管理端批准、拒绝、撤销和发布知识。
- AI 协同端：无设备控制权限，问答文本不能转换为命令。
