...
本目录是四个鸿蒙前端、Spring Boot 后端与 AI 外部服务之间的共同真相源。
...
- [shared-snapshot-contract.md](shared-snapshot-contract.md)：四前端共用的整线/工位快照和动画字段。
...
- [client-capability-matrix.md](client-capability-matrix.md)：每项业务在四前端和后端的对应关系。
...
5. 契约变更 PR 列出受影响的四前端、后端、AI 外部服务、模拟数据和测试。
6. 四个前端不得各自定义同名对象；共享快照、AI 决策、知识审核、状态枚举和命令状态机必须保持字段及语义一致。
