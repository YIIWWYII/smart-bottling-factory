(function () {
  ...
  function renderCompatibilityPlan() {
    ...
    stages.forEach(function (stage) {
      ...
      machine.className = 'compat-machine ' + compatibilityMachineClass(stage.code);
  ...
  function compatibilityMachineClass(stageCode) {
    if (stageCode === 'APPEARANCE_INSPECTION' || stageCode === 'SECONDARY_INSPECTION') return 'camera';
    if (stageCode === 'FILLING' || stageCode === 'BEVERAGE_READY') return 'filling';
    if (stageCode === 'PACKING') return 'robot';
    if (stageCode === 'AGV_TRANSPORT') return 'agv';
    if (stageCode === 'WAREHOUSE_INBOUND') return 'warehouse';
    if (stageCode === 'GAS_INSPECTION') return 'gas';
    return 'machine';
  }
